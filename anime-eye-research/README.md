...
| Path | What it is |
|------|-----------|
| [`prototype/index.html`](prototype/index.html) | **The living design — current best (v4 "liquid-eye").** Full-screen 16:9 animated eyes with a state machine (neutral default + ~12 expressions), auto-blink, idle micro-saccades, breathing, gaze-follow, blush cheeks, and the big glossy dark-orb "cute creature" look. Open it in a browser. |
| [`prototype/candidates/`](prototype/candidates/) | **Design iterations + self-evaluation.** v1 (anime cyan) → v2 (kawaii glow) → v3 (liquid eye) → **v4 (winner)**. See [`EVALUATION.md`](prototype/candidates/EVALUATION.md) for the screenshot-and-judge loop and why v4 is the "million dollar" look. `v4-liquid-eye-SAVED.html` is the pinned current robot version. |

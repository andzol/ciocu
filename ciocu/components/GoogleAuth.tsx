...
// Google's 4-color "G" for our themed sign-in button.
function GoogleG() {
  return (
...
export default function GoogleAuth() {
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      // Rendered but visually hidden (see .google-auth-gis) — it only needs to catch the click.
      // "large" gives a generous hit area under our overlay button.
  ...
  if (!profile) {
    // Google's real button (invisible, handles the click) with our dark-themed button painted on
    // top. GIS/FedCM renders a light button we can't restyle from inside its iframe, so we overlay.
    return (
      <div className="google-auth">
        <div ref={buttonHostRef} className="google-auth-gis" />
        <span className="google-auth-fake" aria-hidden="true">
          <GoogleG />
          Sign in
        </span>
      </div>
